    ...
    public static void sendFortressInfo(Player player, Fortress fort, /*String message, */ float health) {
        try {
            ...
            if (fort.getClanInControl() != null) {
                message = ChatColor.translateAlternateColorCodes('&', "&4&l<&3" + fort.getTag() + "&4&l>&7&o captured by &6&l<&7") + fort.getClanInControl().getColorTag() +  ChatColor.translateAlternateColorCodes('&', "&6&l> &d&l" + (double) Math.round((health / 3) * 10) / 10 + "&7%");
            } else {
